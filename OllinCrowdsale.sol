...
contract OllinCoinCrowdsale is Ownable{
  ...
  uint256 maxTokensToSaleInPreICOPhase = 2000000;
  uint256 maxTokensToSale = 50000000;
  ...
  function OllinCoinCrowdsale(uint256 _startTime, address _wallet, address _tokenToBeUsed) public 
  {
  ...
  function () public  payable {
    ...
    function determineBonus(uint tokens) internal view returns (uint256 bonus) 
    {
        ...
        else if (timeElapsedInDays >= 7 && timeElapsedInDays<22)
        {
            if (TOKENS_SOLD < maxTokensToSale)
            {
                bonus = tokens.mul(10); //10% bonus
                bonus = bonus.div(100);
                require (TOKENS_SOLD.add(tokens.add(bonus)) <= maxTokensToSale);
